...
def stable_task_uids(infos, fallback_uids):
    """Return environment task IDs without changing rollout-group IDs."""
    if len(infos) != len(fallback_uids):
        raise ValueError("environment infos and fallback UIDs must have equal length")
    return np.asarray([
        str(info.get("extra.gamefile") or info.get("task_id") or fallback)
        for info, fallback in zip(infos, fallback_uids)
    ], dtype=object)
...
class TrajectoryCollector:
    ...
    def _vanilla_multi_turn_loop(
        ...
        for _step in range(self.config.env.max_steps):
            ...
            batch.non_tensor_batch['jev_effect_scores'] = np.array(
                [info.get('jev_effect_score', 0.5) for info in infos], dtype=np.float32
            )
            batch.non_tensor_batch['jev_confidences'] = np.array(
                [info.get('jev_confidence', 0.0) for info in infos], dtype=np.float32
            )
    ...
    def _annotate_jev_process_rewards(
        ...
        for batch_index, (batch_rows, info_rows) in enumerate(zip(total_batch_list, total_infos, strict=True)):
            ...
            trajectories.append({
                ...
                'outcome': {
                    'reward': float(episode_rewards[batch_index]),
