...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, gigpo_mode="mean_std_norm", gigpo_enable_similarity=False, gigpo_similarity_thresh=0.95, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            ...
            compute_mean_std_cross_steps=kwargs.get("grpo_cross_steps", True),
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == AdvantageEstimator.OURS:
        advantages, returns, ours_metrics = core_ours.compute_ours_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            step_rewards=data.non_tensor_batch["rewards"],
            action_valids=data.non_tensor_batch["is_action_valid"],
            visible_prompts=data.non_tensor_batch["visible_prompt"],
            anchor_obs=data.non_tensor_batch["anchor_obs"],
            action_text=data.non_tensor_batch["action_text"],
            task_uids=data.non_tensor_batch["uid"],
            traj_uids=data.non_tensor_batch["traj_uid"],
            turn_indices=data.non_tensor_batch["turn_index"],
            minimum_peers=kwargs.get("ours_minimum_peers", 3),
            gamma=gamma,
            invalid_action_penalty=kwargs.get("ours_invalid_action_penalty", 0.1),
            process_weight=kwargs.get("ours_process_weight", 1.0),
            outcome_weight=kwargs.get("ours_outcome_weight", 0.2),
            advantage_clip=kwargs.get("ours_advantage_clip", 3.0),
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, scores, reward_extra_infos_dict, dump_path):
        ...
        n = len(inputs)
        if len(outputs) != n or len(scores) != n:
            raise ValueError("generation inputs, outputs, and scores must have equal length")
        base_data = {
            "input": inputs,
            "output": outputs,
            "score": scores,
    ...
    def _validate_one(self, val_envs, panel_name=None):
        ...
        sample_outputs = []
        sample_scores = []
        sample_extra_infos = defaultdict(list)
        ...
        for test_data in self.val_dataloader:
            ...
            # Validation rollouts are expanded by environment turn. Decode the
            # matching transition prompts rather than the shorter initial batch.
            ...
            for key in (
                ...
                if key not in test_output_gen_batch.non_tensor_batch:
                    continue
                values = test_output_gen_batch.non_tensor_batch[key]
                if len(values) != len(output_texts):
                    continue
                if key == "turn_index":
        ...
        if validation_data_dir:
            ...
            self._dump_generations(
                ...
                reward_extra_infos_dict=sample_extra_infos,
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
                            grpo_cross_steps=self.config.algorithm.get("grpo_cross_steps", True),
                            ...
                            ours_minimum_peers=self.config.algorithm.ours.minimum_peers,
                            ours_invalid_action_penalty=self.config.actor_rollout_ref.actor.invalid_action_penalty_coef,
                        ...
                        if "ours_metrics" in batch.meta_info:
                            metrics.update({
                                f"ours/{key}": value
                                for key, value in batch.meta_info.pop("ours_metrics").items()
                            })
                        if "jev_step_metrics" in batch.meta_info:
                            metrics.update({
                                f"jev_step/{key}": value
                                for key, value in batch.meta_info.pop("jev_step_metrics").items()
                            })
