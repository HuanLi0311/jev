...
def compute_jev_step_grpo_advantage(
    ...
    for batch_idx, (task, traj, turn) in enumerate(
        ...
        outcome = float(outcomes[batch_idx])
        if key in records:
    ...
    for key, record in records.items():
        trajectories[(record["task"], record["traj"])].append(key)

    for keys in trajectories.values():
        keys.sort(key=lambda item: item[2])
        outcome = records[keys[0]]["outcome"]
        for key in keys:
            record = records[key]
            if abs(record["outcome"] - outcome) > epsilon:
                raise ValueError("episode outcome is inconsistent within a trajectory")

    turn_groups = defaultdict(list)
    for key, record in records.items():
        turn_groups[(record["task"], record["turn"])].append(key)
    ...
    outcome_equivalent_keys = {
        key
        for keys in turn_groups.values()
        if len({round(records[key]["outcome"], 12) for key in keys}) == 1
        for key in keys
    }
    ...
    stats = {
        ...
        "outcome_equivalent_nonzero_fraction": (
            active_outcome_equivalent / len(outcome_equivalent_keys)
