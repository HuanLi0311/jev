...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        # ponytail: SDPA works without flash_attn; keep FlashAttention2 when installed.
        attention_backend = "flash_attention_2" if find_spec("flash_attn") else "sdpa"
        actor_model_config = AutoConfig.from_pretrained(local_path, trust_remote_code=trust_remote_code, attn_implementation=attention_backend)
    ...
    def begin_rollout(self):
        assert self._is_rollout and not self._rollout_open
        self.rollout_sharding_manager.__enter__()
        self._rollout_open = True

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def end_rollout(self):
        if self._rollout_open:
            try:
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with nullcontext() if self._rollout_open else self.rollout_sharding_manager:
